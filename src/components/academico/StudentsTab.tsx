...
const PAGE_SIZE = 20; // Número de alunos por requisição
...
export function StudentsTab() {
  ...
  // Recarrega alunos quando o termo de busca mudar (reset da lista)
  useEffect(() => {
    if (user) {
      loadStudents(true); // true = resetar paginação
    }
  }, [searchTerm]);
  ...
  const loadStudents = async (reset: boolean = false) => {
    if (!user) return;

    // Se for reset, reinicia a paginação
    ...
    try {
      ...
      // Busca no servidor (case-insensitive) se houver termo
      if (searchTerm.trim()) {
        query = query.or(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      resetForm();
      loadStudents(true); // Recarrega a lista (primeira página)
    } catch (error: any) {
